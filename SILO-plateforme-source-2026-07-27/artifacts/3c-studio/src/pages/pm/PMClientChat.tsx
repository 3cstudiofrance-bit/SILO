...
export default function PMClientChat() {
  ...
  const conversations = projects.map(p => ({
    ...
    clientName: p.clientName ?? p.clientEmail ?? "Client",
    initials: (p.clientName ?? p.title).slice(0, 2).toUpperCase(),
  }));
    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center py-32 text-center">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">Vos conversations client apparaîtront ici.</p>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="flex h-[calc(100vh-3.5rem-2.5rem)] -m-5 md:-m-8 rounded-xl overflow-hidden border border-border">
        <div className="w-72 flex-shrink-0 border-r border-border bg-card flex flex-col">
          <div className="p-4 border-b border-border">
            <h2 className="text-sm font-semibold text-foreground mb-3">Discussion client</h2>
            ...
            {filtered.map(conv => (
              <button key={conv.id} onClick={() => setSelectedId(conv.id)}
                className={cn("w-full flex items-center gap-3 px-4 py-3 text-left transition-colors",
                  current?.id === conv.id ? "bg-primary/10 border-l-2 border-primary" : "hover:bg-secondary border-l-2 border-transparent")}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground/70 truncate">{conv.projectTitle}</p>
                </div>
              </button>
            ))}
          </div>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-card">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground/70">{current.projectTitle}</p>
                </div>
              </div>
            </div>
          )}
